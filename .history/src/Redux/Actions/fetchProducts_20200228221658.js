...
export const fetchProducts = () => {
    return dispatch => {
        ...
        fetch("http://localhost:5000/.netlify/functions/server/products/shoes", {
                method: 'GET'
            })
            .then(res => res.json())
            .then(res => {
...
export const updateQuantityProducts = (id, quantityOut) => {
    return dispatch => {
        console.log(quantityOut, "bingo")
        dispatch(updateProductsPending());
        fetch("http://localhost:5000/.netlify/functions/server/products/shoes/update" + id, {
                method: 'POST',
                headers: {
